...
#This is an explanation of why the plot is difficult to read, not why the data are difficult to use. The x and y axes also have consistent values are accurately represent relative distance.
...
#What two sets? I'm not sure what correlation you are talking about...and why are you confident in it?
...
# Test the significance of both models and look at the model summary. (3 points each)
  ...
  #Yield with Env seems to be the best model. When plotted linearly, Yield with Gen seems to be very over fitted. (Run codes above) #these are not scatter plots (i.e. not linear). How do the r-squared and p values compare?
...
#The worse choice ould be the the Genotype yield response as there is no clear linear response. #Genotype is not one of the environments.